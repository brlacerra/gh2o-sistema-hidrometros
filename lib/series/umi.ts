...
import { addMonthsMs, monthBucketStartMs, monthRangeBoundsMs } from "./monthBucket";

export type UmiStatsMode = "avg" | "minavgmax";

export type UmiPoint =
  | { tMs: number; avg: number | null }
  | { tMs: number; avg: number | null; min: number | null; max: number | null };

export function buildUmiSeries(opts: {
  fromMs: number;
  toMs: number;
  bucketMs: number; // para 1mo pode ser -1
  readings: Array<{ ts: bigint; umiAvg: number | null }>;
  tz?: string;
  bucketKind?: "fixed" | "day" | "month";
  stats?: UmiStatsMode;
  minPoints?: number; // estende end para frente (igual você fez)
}): { points: UmiPoint[] } {
  const { fromMs, toMs, bucketMs, readings } = opts;
  const tz = opts.tz ?? "America/Sao_Paulo";
  const bucketKind = opts.bucketKind ?? "fixed";
  const stats: UmiStatsMode = opts.stats ?? "avg";
  ...
  for (const r of readings) {
    ...
    const cur = agg.get(b);
    if (!cur) agg.set(b, { min: v, max: v, sum: v, count: 1 });
    else {
  ...
  const points: UmiPoint[] = [];

  if (bucketKind === "month") {
    let { start, end } = monthRangeBoundsMs(fromMs, toMs, tz);

    // minPoints: estender para frente
    if (minPoints > 0) {
      let count = 0;
