...
// helpers month/day bucket like we used for rain/lum
...
function build15mTempFromRows(rows: Array<{ bucket_s: bigint; avg: any }>) {
  return rows.map((r) => ({ tMs: Number(r.bucket_s) * 1000, avg: numOrNull(r.avg) }));
}

// build 1d min/avg/max series (no fill)
function build1dTempFromRows(rows: Array<{ bucket_s: bigint; min: any; avg: any; max: any; std: any }>) {
  return rows.map((r) => ({
...
// fill daily series aligned with dayBucketStartMs (uses dayRangeBoundsMs/dayBucketStartMs)
function buildDailyTempFilled(opts: {
  rows: Array<{ bucket_s: bigint; min: any; avg: any; max: any; std: any }>;
  startMs: number;
  endMs: number;
  minPoints?: number;
  tz?: string;
  ...
  const byBucket = new Map<number, { min: number | null; avg: number | null; max: number | null; std: number | null }>();
  for (const r of rows) {
    const b = Number(r.bucket_s);
    byBucket.set(b, { min: numOrNull(r.min), avg: numOrNull(r.avg), max: numOrNull(r.max), std: numOrNull(r.std) });
  }

  // align start/end using dayRangeBoundsMs (returns ms)
  ...
  const points: TempPointMinAvgMax[] = [];
  let iters = 0;
  ...
  for (let t = alignedStartMs; t <= endAdjustedMs; t += DAY_MS) {
    ...
    const bucketStartMs = dayBucketStartMs(t, tz); // ms aligned
...
// fill monthly min/avg/max series
function buildMonthlyTempFromRows(opts: {
  rows: Array<{ bucket_s: bigint; min: any; avg: any; max: any; std: any }>;
  startMs: number;
  endMs: number;
  minPoints?: number;
...
export async function GET(_req: Request, ctx: { params: Promise<{ codSta: string }> }) {
  ...
  const [rows15m24h, rows1d30d, rows1moHydroYear] = await Promise.all([
    ...
    `,
    prisma.$queryRaw<Array<{ bucket_s: bigint; min: any; avg: any; max: any; std: any }>>`
      SELECT
        (FLOOR((ts + ${SP_OFFSET_S}) / ${BUCKET_1D_S}) * ${BUCKET_1D_S} - ${SP_OFFSET_S}) AS bucket_s,
        MIN(tempAvg) AS min,
        AVG(tempAvg) AS avg,
        MAX(tempAvg) AS max,
        STDDEV(tempAvg) AS std
      FROM data
      WHERE codSta = ${codSta}
  ...
  // build series
  const points15m24h = build15mTempFromRows(rows15m24h);
  const points1d30d = build1dTempFromRows(rows1d30d);

  const day = points15m24h.filter((p) => p.tMs >= fromDayMs);
  const last24h = points15m24h;

  // fill month days using day bucket alignment
  const month = buildDailyTempFilled({
    rows: rows1d30d,
    startMs: fromMonthMs,
    endMs: nowMs,
    // you can set minPoints: 31 to always show 31 columns if wanted
  });

  const last30d = points1d30d;

  // monthly (year) series with minPoints
  const year = buildMonthlyTempFromRows({
  ...
  const hydroYear = buildMonthlyTempFromRows({
    rows: rows1moHydroYear,
